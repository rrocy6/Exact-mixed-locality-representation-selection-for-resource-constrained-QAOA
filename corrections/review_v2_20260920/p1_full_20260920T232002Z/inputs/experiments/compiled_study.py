...
CONFIG = {
    ...
    "candidate_budget_rule": "min(32, number of capacity-feasible full assignments), identical for both spaces",
...
def design_libraries(instance):
    ...
    budget=min(CONFIG["candidate_budget_per_space"],len(full))
    ...
    for topo in CONFIG["topologies"]:
        ...
        for strategy in CONFIG["strategies"]:
            ...
            results.append({"topology":topo,"strategy":strategy,
                ...
                "scheduled_evaluations_per_space":len(lib["full"])})
...
def run(workers):
    ...
    if frozen["source_sha256"]!=sha(__file__):
        amendment=json.loads((OUT/"amendment_01.json").read_text())
        assert amendment["original_source_sha256"]==frozen["source_sha256"]
        assert amendment["amended_source_sha256"]==sha(__file__)
        assert amendment["config"]==CONFIG
    else: assert frozen["config"]==CONFIG
