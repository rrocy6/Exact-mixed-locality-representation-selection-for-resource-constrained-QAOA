    ...
    for width, group in groups.items():
        ...
        try:
            ...
            for task, circuit in group:
                ...
                if task.get('semantic') or attempt_id.startswith('repeat_') or attempt_id == 'seed_2718':
...
def run_formal(run: Path, workers: int | None = None) -> None:
    ...
    if pending:
        ...
        report = read_json(run / 'P1_REPORT.json')
        if report.get('status') != 'pass' or report.get('identity') != execution_identity(run, config):
            raise RuntimeError('Matching successful P1 evidence required before compilation')
        if report.get('rows_sha256') != sha256_file(run / 'p1_repetition_rows.csv'):
            raise RuntimeError('P1 evidence changed')
        try:
...
def p1(run: Path) -> None:
    config, samples, candidates, _ = load_run(run)
    require_current_compiler(run, config)
    selected = [row for row in samples if str(row["selected"]).lower() == "true"]
    first_by_family = {}
    for family in FAMILIES:
        family_rows = [row for row in selected if row["family"] == family]
        first_by_family[family] = min(family_rows, key=lambda row: (row["selection_hash"], row["instance_id"]))["instance_id"]
    target_groups = defaultdict(list)
    for row in candidates:
        if row['instance_id'] in first_by_family.values():
            target_groups[(row['family'], row['topology'], row['synthesis'], row['category'])].append(row)
    target_candidates = [min(group, key=lambda c: (int(c['logical_Q']), c['candidate_id'])) for group in target_groups.values()]
    results = {}
    validation_errors = []
    if (run / "P1_REPORT.json").exists():
        raise RuntimeError("P1 evidence exists: create a new run for a new attempt")
    started = time.perf_counter()
    for repeat, seed in (("repeat_a", 1729), ("repeat_b", 1729), ("seed_2718", 2718)):
        batch_groups: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
        for candidate in target_candidates:
            batch_groups[(candidate["instance_id"], candidate["topology"], candidate["synthesis"])].append(candidate)
        jobs = []
        for (instance_id, topology, synthesis), group in sorted(batch_groups.items()):
            tasks = []
            for candidate in group:
                task_key = {"instance_id": instance_id, "candidate_id": candidate["candidate_id"], "topology": topology, "synthesis": synthesis, "seed": seed}
                tasks.append({"task_id": task_id(task_key), **task_key, "candidate": candidate})
            jobs.append((str(run), tasks, seed, config["compiler"]["sabre_trials"], repeat))
        rows = []
        for job in jobs:
            result = preflight_job(job, config)
            try:
                validate_rows(job[1], result['rows'], target_candidates, require_success=True)
    ...
    for repeat, rows in results.items():
        for row in rows:
            by_key.setdefault((row["instance_id"], row["candidate_id"], row["topology"], row["synthesis"]), {})[repeat] = row
    mismatches = []
    for key, repeats in by_key.items():
        left, right = repeats.get("repeat_a"), repeats.get("repeat_b")
        if not left or not right or left["status"] != right["status"] or left.get("resources_json") != right.get("resources_json"):
            mismatches.append(key)
    p1_rows = []
    for repeat, rows in results.items():
        for row in rows:
            p1_rows.append({"repeat": repeat, **row})
    fields = sorted({key for row in p1_rows for key in row})
    write_csv(run / "p1_repetition_rows.csv", p1_rows, fields)
    estimate = (time.perf_counter() - started) / max(1, len(p1_rows)) * sum(1 for row in candidates if row["instance_id"] in {item for item in first_by_family.values()}) * len(SEEDS) / 2
    if mismatches or validation_errors:
        atomic_json(run / 'P1_REPORT.json', {'status': 'fail', 'identity': execution_identity(run, config), 'validation_errors': validation_errors, 'repeatability_mismatches': mismatches})
        atomic_json(run / "RUN_STATE.json", {"run_id": run.name, "stage": "p1_failed", "status": "fail", "repeatability_mismatch_count": len(mismatches), "mismatches": mismatches[:20]})
        raise RuntimeError(f"P1 repeated seed mismatch: {len(mismatches)}")
    atomic_json(run / "P1_REPORT.json", {"status": "pass", "identity": execution_identity(run, config), "rows_sha256": sha256_file(run / "p1_repetition_rows.csv"), "first_instance_by_family": first_by_family, "repeat_rows": len(p1_rows), "selection_rule": "first frozen instance per family; minimum logical width then candidate ID per topology/synthesis/category", "repeatability_mismatch_count": 0, "seed_2718_present": any(row["repeat"] == "seed_2718" for row in p1_rows), "estimated_formal_seconds": estimate, "estimate_note": "two-instance pilot extrapolation; full design uses measured batch throughput", "completed_utc": datetime.now(timezone.utc).isoformat()})
    atomic_json(run / "RUN_STATE.json", {"run_id": run.name, "stage": "p1_pass", "status": "pass", "repeatability_mismatch_count": 0, "estimated_formal_seconds": estimate, "formal_scope": "full 30+30 because T_cap_seconds is null", "updated_utc": datetime.now(timezone.utc).isoformat()})
