...
\begin{abstract}
Higher-order pseudo-Boolean objectives provide compact formulations for combinatorial optimisation, but in QAOA they induce a trade-off between interaction locality, auxiliary qubits, penalty strengths, and compiled circuit cost. We develop an exact selective locality-reduction framework in which only chosen higher-order terms are reduced. For arbitrary-degree objectives, we derive a sharp aggregate-residual penalty for a single product substitution and prove that successive substitutions preserve pointwise exactness. For cubic objectives, this gives a closed-form mixed-sign threshold for auxiliaries shared across multiple terms.

We then formulate representation selection over the resulting exact design space using compiled two-qubit resources, auxiliary count, penalty scale, and a relaxation-derived fibre-risk constraint. A finite anytime search returns feasible designs and, when certification is enabled, an optimality-gap bound. The same relaxation is also used to construct representation-dependent warm starts for original and auxiliary qubits.

Experiments on Max-3SAT and cubic spin-glass instances verify exactness and penalty tightness, validate the selector against exhaustive search, and compare native, fully reduced, selective, and matched-random representations under common compilation and QAOA budgets. Selective reduction gives the clearest circuit-resource gains for Max-3SAT, but lower circuit cost does not translate into a universal finite-depth QAOA advantage. On a deliberately signal-enriched Max-3SAT subset, relaxation-informed warm starts improve on cold start. The original E4 and spin-glass comparisons remain inconclusive, as does the additional value of direct auxiliary pair moments over the independence closure. Overall, selective reduction provides a controlled way to trade locality against resources while preserving the represented objective exactly.
\end{abstract}
...
A pseudo-Boolean optimisation problem is written as
\begin{equation}
    \min_{x\in\{0,1\}^{n}} f(x),
    \qquad
    ...
    \label{eq:intro-pbo}
\end{equation}
Higher-order terms arise naturally in clause penalties, hypergraph objectives,
and many-body spin models, and can encode such structure more compactly than an
imposed quadratic formulation~\cite{BorosHammer2002,Hadfield2021,Glos2022,
CampbellDahl2022}.  For QAOA~\cite{Farhi2014}, however, the polynomial
representation is also a circuit-design choice.  Under the standard diagonal
map $x_i\mapsto N_i=(I-Z_i)/2$, a degree-$k$ monomial becomes a diagonal
interaction of locality at most $k$.  Keeping a higher-order term therefore
avoids auxiliary variables, but its phase separator must still be synthesised
into the native gate set; reducing the term lowers its logical locality at the
cost of extra qubits, penalty couplings, and a modified interaction graph.
The relevant question is consequently not whether high or low locality is
better in isolation, but which representation is preferable for a given
problem and compilation target.

This trade-off already appears in several forms.  Higher-order QAOA has been
studied on structured encodings, random hypergraphs, and spin-glass models, and
has been implemented in short-depth hardware-tailored circuits
~\cite{Glos2022,CampbellDahl2022,Basso2022,PelofskeShortDepth2024,
Pelofske2024}.  Comparisons between higher-order and quadratic endpoint
encodings show that reducing locality can increase qubit count while changing
logical and compiled depth, two-qubit gate count, and routing cost
~\cite{Stein2023,Bell2025,Verchere2023,Weidenfeller2022,Schmidbauer2024,
Koch2026}.  The preferred endpoint is therefore problem and
hardware dependent rather than universal.  Related width coupling trade-offs
also occur in quantum annealing~\cite{Nagies2025}, although the dynamics are
different from those considered here.

The standard mechanism for lowering locality is \emph{quadratization}.
Rosenberg type substitutions replace products by auxiliary binary variables
and add penalties enforcing the intended relations~\cite{Rosenberg1975}.
A large literature develops exact quadratizations with different auxiliary and
coefficient requirements, including sign-specific, aggregative, compact, and
optimisation-based constructions~\cite{FreedmanDrineas2005,Ishikawa2011,
BorosGruber2014,AnthonyEtAl2017,BorosCramaRodriguezHeck2020,Dattani2019,
VermaLewis2020,VermaLewisKochenberger2021,CramaEtAl2025}.  The relevant
notion of exactness is pointwise:
\begin{equation}
    f(x)=\min_{y\in\{0,1\}^{m}}F(x,y)
    \qquad\text{for every }x\in\{0,1\}^{n}.
    \label{eq:intro-pointwise}
\end{equation}
This is stronger than preserving only the global optimum.  It fixes the lower
envelope of the reformulated objective on every original assignment and hence
preserves the represented classical problem.  It does not, however, make two
QAOA representations dynamically equivalent.  The enlarged Hamiltonian also
contains auxiliary-inconsistent basis states whose energies depend on the
penalties, so its excited spectrum and finite-depth variational dynamics may
change even when Eq.~\eqref{eq:intro-pointwise} holds.

Most existing reduction methods choose a complete quadratic representation.
Substitution selection has been used to reduce auxiliary count or penalty
magnitude~\cite{VermaLewis2020,VermaLewisKochenberger2021}, to control graph
structure~\cite{Herrman2021,Schmidbauer2026}, and, more recently, to choose
auxiliaries that compile efficiently on sparse quantum hardware
~\cite{Rovara2026}.  These methods still require all higher-order terms to be
removed.  At the opposite extreme, approximate ancilla-free reductions can
shorten circuits by deliberately changing the higher-order generator
~\cite{Dragoi2026}.  The intermediate exact design problem remains largely
open: if pointwise exactness is held fixed, which interactions should remain
native, which should be reduced, and how should reusable auxiliaries and their
penalties be chosen in view of compiled circuit cost?

We address this problem through \emph{exact selective locality reduction}.
Only a chosen family of higher-order terms is reduced; the remaining terms are
kept at their original locality.  A product auxiliary may be reused by several
selected terms, so its penalty must control their \emph{collected} residual
rather than any coefficient separately.  For an arbitrary-degree current
objective, we show that a single Rosenberg substitution is pointwise exact if
and only if
\(    M\geq \lVert A\rVert_{\infty},
\)
where $A$ is the aggregate residual multiplying the substituted product.  The
condition is sharp.  We then prove that these one-step identities compose
through any admissible sequence of substitutions, yielding an exact reduction
of any selected family to a prescribed target locality.  In the cubic case the
residual is linear, and the sharp threshold for a pair $p$ shared by selected
terms has the closed form
\[
    T_p=
    \max\!\left\{
        \sum_{A\in D_p:\,c_A>0} c_A,
        \sum_{A\in D_p:\,c_A<0} |c_A|
    \right\}.
\]
Thus the all-native and fully quadratized formulations are only the two
endpoints of a finite family of pointwise-exact mixed quadratic--cubic
representations.

Once exactness is fixed, representation choice becomes a resource-selection
problem.  We score candidate representations using auxiliary count, compiled
two-qubit gate count and depth, and maximum penalty scale under a declared
compiler protocol.  A relaxation-derived fibre-excess quantity is used as a
guardrail against representations whose auxiliary relations are poorly aligned
with the same relaxation data.  The resulting finite search can be run as a
practical Pareto beam and, on instances where certification is required, as an
anytime branch-and-bound procedure with a valid optimality gap bound.  The aim
is not to introduce a new classical solver for the design problem, but to make
the choice of quantum representation explicit, reproducible, and certifiable
relative to a stated resource model.

The same relaxation also supplies a representation dependent warm start.
Relaxation informed QAOA warm starts normally assume a fixed register
~\cite{Egger2021,TateSDP2023,TateCustom2023}.  Here the register changes with
the selected representation, so every active product auxiliary requires its
own initial marginal and matched mixer.  We use one common relaxation to
initialise both original and auxiliary qubits and to define the fibre-risk
diagnostic used during selection. This is an ancilla-adapted use of existing warm-start ideas rather than a new warm-start family. We develop the exactness theory at arbitrary degree, while the representation-selection and numerical analyses focus on cubic-to-quadratic reduction as the first fully explicit case.
The main contributions are as follows.
\begin{enumerate}
    \item We derive a necessary and sufficient aggregate residual penalty for a
    single product substitution and prove compositional pointwise exactness for
    successive substitutions.  This gives an arbitrary-degree selective
    reduction to any prescribed target locality.

    \item For cubic objectives, we obtain a closed-form mixed-sign penalty for
    auxiliaries shared across several terms.  The construction defines an exact
    mixed quadratic--cubic design space containing the native and fully
    quadratized formulations as endpoints.

    \item We formulate compiler and relaxation aware selection over this exact
    design space.  A finite anytime search returns feasible representations and,
    when certification is enabled, a rigorous optimality-gap bound relative to
    the declared compiler, budgets, and resource objective.

    \item We evaluate the framework on Max-3SAT and cubic spin-glass instances
    using native, fully reduced, selective, and auxiliary-count-matched random
    representations.  Exhaustive tests confirm the sharp penalty behaviour, and
    oracle-tier branch-and-bound closes the certified gap on all tested small
    instances.  Selective reduction gives the clearest compiled-resource gains
    for Max-3SAT across the tested connectivity targets, whereas the spin-glass
    data exhibit a gate depth trade-off.  These resource gains do not produce a
    universal finite-depth QAOA advantage.  On a relaxation-signal-enriched
    Max-3SAT subset, warm starting improves over cold start, while the additional
    benefit of direct auxiliary pair moments over the independence closure remains inconclusive.
\end{enumerate}

The remainder of the paper is organised as follows.  Section~2 fixes the
pseudo-Boolean, circuit, and relaxation notation.  Section~3 develops the
general exactness theory and its cubic specialisation.  Section~4 formulates
representation selection and certification, and Section~5 gives the
representation-dependent warm start.  Sections~6 and~7 describe the benchmark
families and experimental protocol.  Sections~8 and~9 present the numerical
results and their interpretation, and Section~10 concludes.
...
Let \(\mathbb B:=\{0,1\}\), \([n]:=\{1,\ldots,n\}\), and
\(x_A:=\prod_{i\in A}x_i\), with \(x_\varnothing:=1\).  Every function
\(f:\mathbb B^n\to\mathbb R\) has the unique multilinear expansion
\begin{equation}
...
\end{equation}
where \(\mathbf 1_B\) denotes the characteristic vector of \(B\)
\cite{BorosHammer2002,Hadfield2021}.
Its monomial support and degree are
\begin{equation}
...
\end{equation}
For a target locality \(\ell\geq2\), write
\begin{equation}
  \mathcal{M}_{>\ell}(f):=
  \{A\in\operatorname{msupp}(f):|A|>\ell\},
  \qquad
  f_{>\ell}:=\sum_{A\in\mathcal M_{>\ell}(f)}c_Ax_A,
  \qquad f_{\leq\ell}:=f-f_{>\ell}.
  \label{eq:target-split}
\end{equation}
Selective locality reduction modifies only a chosen subfamily of
\(\mathcal M_{>\ell}(f)\); the case \(\ell=2\) is selective quadratization.

On \((\mathbb C^2)^{\otimes n}\), set \(N_i:=(I-Z_i)/2\) and
\(N_A:=\prod_{i\in A}N_i\).  The diagonal cost Hamiltonian is
\begin{equation}
...
\end{equation}
Indeed, \(N_i|x\rangle=x_i|x\rangle\), so substitution gives
Eq.~\eqref{eq:diagonal-cost}.  The computational basis is a complete
eigenbasis, hence \(\lambda_{\min}(H_f)=\min_x f(x)\).  One QAOA cost layer acts as
\begin{equation}
...
\end{equation}
With \(Z_B:=\prod_{i\in B}Z_i\), the collected Pauli coefficients are
\cite{Hadfield2021}
\begin{equation}
...
\end{equation}
For nonconstant \(f\), the largest nonzero Pauli weight is \(\deg(f)\).

Thus reduction lowers logical locality but introduces auxiliary qubits and
penalty terms.  We assume no native higher-order hardware interaction
~\cite{Farhi2014,CampbellDahl2022,Schmidbauer2024}; the reference synthesis
is specified in Appendix~\ref{app:reference-compiler}.

\subsection{Pointwise-exact auxiliary reformulations}
\label{sec:pointwise-exactness}

Let \(y\in\mathbb B^m\) be auxiliary variables. A function
\(F:\mathbb B^{n+m}\to\mathbb R\) is a \emph{pointwise-exact reformulation}
of \(f\) if
\begin{equation}
  f(x)=\min_{y\in\mathbb B^m}F(x,y)
  \qquad (x\in\mathbb B^n).
  \label{eq:pointwise-exactness}
\end{equation}
It is a quadratization if, in addition, \(\deg(F)\leq2\)
\cite{BorosGruber2014,AnthonyEtAl2017,CramaEtAl2025}.  For each \(x\), the
corresponding optimal auxiliary fibre is
\begin{equation}
  \mathcal{Y}_F(x):=\operatorname*{arg\,min}_{y\in\mathbb{B}^m}F(x,y).
  \label{eq:optimal-fibre}
\end{equation}
If \(\mathcal C(x)\subseteq\mathbb B^m\) is the set of assignments satisfying
the intended product relations, then \(F\) is \emph{constraint-faithful} when
\(\mathcal Y_F(x)\subseteq\mathcal C(x)\) for all \(x\).  If
\(\mathcal C(x)=\{\phi(x)\}\), this condition is equivalent to
\(\mathcal Y_F(x)=\{\phi(x)\}\).
Equation~\eqref{eq:pointwise-exactness} implies, with
\(\pi_x(x,y):=x\),
\begin{equation}
  F(x,y)\geq f(x),
  \quad
...
\end{equation}
and, writing \(f^\star:=\min_x f(x)\),
\begin{equation}
  0\leq f(x)-f^\star\leq F(x,y)-f^\star.
  \label{eq:gap-ordering}
\end{equation}
\begin{proof}
For each \(x\), \(F(x,y)\geq\min_zF(x,z)=f(x)\), with equality for
some \(y\) because the fibre is finite.  Minimising over \(x\) gives
\(\min_{x,y}F=f^\star\).  If \((x,y)\) minimises \(F\), then
\(f^\star\leq f(x)\leq F(x,y)=f^\star\), so \(x\) minimises \(f\).
Conversely, a minimiser \(x\) of \(f\), together with any
\(y\in\mathcal Y_F(x)\), satisfies \(F(x,y)=f^\star\).
This proves the projection identity; subtracting \(f^\star\) from
\(F(x,y)\geq f(x)\geq f^\star\) gives Eq.~\eqref{eq:gap-ordering}.
\end{proof}
Pointwise exactness does not imply spectral or finite-depth QAOA equivalence: \(H_F\) also acts on assignments
outside the optimal fibres.

For \(a,b,y\in\mathbb B\), the Rosenberg penalty~\cite{Rosenberg1975} is
\begin{equation}
  \begin{gathered}
  P_{\mathrm R}(a,b,y):=ab-2ay-2by+3y,\\
  P_{\mathrm R}(a,b,y)=0\iff y=ab,
  \qquad P_{\mathrm R}(a,b,y)\geq1\ \text{otherwise}.
  \end{gathered}
  \label{eq:rosenberg}
\end{equation}
Section~\ref{sec:cubic-quadratization} determines the
sharp coefficient multiplying this penalty and proves exactness under
successive substitutions.

\subsection{Representation, resource, and relaxation data}
\label{sec:resource-relaxation-data}

An admissible representation \(\sigma\) is a pointwise-exact function
\begin{equation}
...
\end{equation}
where \(u=(x,y)\), \(u_S:=\prod_{i\in S}u_i\), \(a_\sigma\) is the auxiliary
count, and \(L_\sigma\) is the retained locality.  The unchanged objective is
the native representation;
reducing all terms in \(\mathcal M_{>\ell}(f)\) gives a fully reduced
representation, while reducing a proper subfamily gives a selective one.  Its
quadratic interaction graph is
\(
...
\)
higher-order supports are treated as hyperedges.

Fix a hardware architecture \(\mathcal A\) and compilation protocol
\(\mathcal C\).  Compilation of one cost layer defines
\(
...
\)
where the entries are the two-qubit gate count, two-qubit depth, and inserted
SWAP count~\cite{Weidenfeller2022,Schmidbauer2024,Rovara2026}.  For active
penalties \(\{M_{r,\sigma}\}\), set
\(M_{\max,\sigma}:=\max_r M_{r,\sigma}\), with value zero when no penalty is active.  For the collected Pauli expansion
\(H_{F_\sigma}=\sum_T h_T^{(\sigma)}Z_T\), its non-constant coefficient
dynamic range is
\begin{equation}
  \kappa_\sigma:=
...
\end{equation}
This quantity is invariant under nonzero global rescaling. A larger \(\kappa_\sigma\) indicates a wider coefficient dynamic range and hence a potentially more demanding Hamiltonian to implement accurately.

Finally, write \(\bar\mu_i\in[0,1]\) and \(\bar q_{ij}\in[0,1]\) for the
raw first and pair pseudo-moments supplied by the relaxation, with pair data
used when available.  The probabilities \(\mu_i,q_p\) derived from them enter the selector in
Section~\ref{sec:selection} and the warm start in
Section~\ref{sec:warm-start}~\cite{Egger2021,TateSDP2023,TateCustom2023}.
The raw moments need not arise from a common joint distribution; the
independent product law used for the diagnostic is introduced separately.

\section{Selective Locality Reduction and Cubic Quadratization}
\label{sec:cubic-quadratization}
We first develop pointwise-exact selective locality reduction for pseudo-Boolean objectives of arbitrary degree. Proposition~\ref{prop:sharp-aggregate-residual} gives the sharp penalty for one substitution, while Theorem~\ref{thm:sequential-compositional-exactness} and Corollary~\ref{cor:selective-target-locality} extend the construction to successive substitutions and arbitrary target locality. We then specialise to cubic-to-quadratic reduction, for which the penalty and exact representation space admit an explicit form. The construction builds on Rosenberg substitutions and auxiliary reuse \cite{Rosenberg1975,BorosGruber2014,VermaLewisKochenberger2021,CramaEtAl2025}.


\subsection{General one-step product substitution}
\label{sec:one-step-substitution}

Let \(q:\mathbb B^N\to\mathbb R\) be a collected multilinear polynomial in
\(u=(u_1,\ldots,u_N)\), where \(u_T:=\prod_{k\in T}u_k\).  Fix
\(p=\{i,j\}\in\binom{[N]}2\) and a selected family
\begin{equation}
  \mathcal{S}_p\subseteq
  \{S\in\operatorname{msupp}(q):p\subseteq S,\ |S|\geq3\}
  \label{eq:selected-family-one-step}
\end{equation}
whose common factor is to be replaced.  If \(d_S\) denotes the coefficient of
\(u_S\) in \(q\), define, for \(v:=(u_k)_{k\notin p}\),
\begin{equation}
  \begin{aligned}
A_p(v)&:=\sum_{S\in\mathcal{S}_p}d_Su_{S\setminus p},
    &B_p(u)&:=q(u)-u_i u_jA_p(v),\\
    q(u)&=B_p(u)+u_i u_jA_p(v).&&
  \end{aligned}
  \label{eq:aggregate-residual-decomposition}
\end{equation}
Multilinearity makes \(A_p\) independent of \(u_i,u_j\).  Introduce
\(y_p\in\mathbb B\), intended to equal \(u_i u_j\), and set
\begin{equation}
  \begin{aligned}
    Q_{p,M_p}(u,y_p)
...
\end{equation}
Thus the exact energy change is
\[
  Q_{p,M_p}(u,y_p)-q(u)
  =(y_p-u_i u_j)A_p(v)+M_pP_{\mathrm R}(u_i,u_j,y_p).
\]
The inconsistent choices can lower the substituted block by at most
\(\|A_p\|_\infty\), while their Rosenberg penalty is at least one.
The following proposition gives the exact threshold and separation margin.
...
\begin{equation}
  ...
  \quad\text{for all }u\in\mathbb B^N
...
\end{equation}
In addition, \(y_p=u_i u_j\) is the unique minimiser for every \(u\) if and only
if \(M_p>\|A_p\|_\infty\).
\end{proposition}

The proof in Appendix~\ref{app:sharp-one-step-proof} checks the two values of
\(y_p\).  The threshold depends on the sum of all selected terms sharing
\(y_p\), rather than on the largest individual coefficient.
...
Suppose \(M_r\geq\|A_r\|_\infty\) for every \(r\in[R]\).  Then, for every
\((x,y)\in\mathbb B^{n+R}\),
\begin{equation}
  g^{(R)}(x,y)-f(x)
  \geq
...
The one-step gaps telescope, and every penalty vanishes on the recursive
lift; Appendix~\ref{app:compositional-proof} gives the proof.  Equality
thresholds preserve the lower envelope but may admit ties.  Every acyclic
substitution order is valid, although its penalties, reuse pattern, and
compiled resources may differ.
...
A fresh product chain reduces each selected degree-\(k\) monomial in
\(k-\ell\) steps, each with residual norm \( |c_A|\); see
Appendix~\ref{app:target-locality-proof} for the construction and proof.
Shared intermediate products can improve the bound in
Eq.~\eqref{eq:target-locality-bound}, but then each aggregate residual must be
recomputed before applying Proposition~\ref{prop:sharp-aggregate-residual}.

\subsection{Cubic specialisation}
\label{sec:cubic-specialisation}

The preceding results apply to arbitrary degree and any target locality \(\ell\geq2\). We now specialise to \(\deg(f)\leq3\) and \(\ell=2\), the first case in which the sharp penalties and complete representation space are both explicit. Write
\begin{equation}
  f(x)=f_{\leq2}(x)+\sum_{A\in\mathcal C_3(f)}c_Ax_A,
  \qquad
  \mathcal C_3(f):=\{A\subseteq[n]:|A|=3,\ c_A\neq0\}.
  \label{eq:cubic-decomposition}
\end{equation}
Choose \(\mathcal D\subseteq\mathcal C_3(f)\), put
\(\mathcal R:=\mathcal C_3(f)\setminus\mathcal D\), and assign each
\(A\in\mathcal D\) a pair \(\rho(A)\in\binom{A}{2}\).  Define
\[
...
\]
For each \(p=\{i,j\}\in P_\rho\), introduce \(y_p\) with intended value
\(x_ix_j\).  The mixed representation is
\begin{equation}
  F_{\rho,M}(x,y)
  =f_{\leq2}(x)+\sum_{A\in\mathcal R}c_Ax_A
   +\sum_{p\in P_\rho}y_pA_p(x)
   +\sum_{p=\{i,j\},\,p\in P_\rho}
    M_pP_{\mathrm R}(x_i,x_j,y_p).
  \label{eq:cubic-mixed-objective}
\end{equation}
Because \(A\setminus p\) is a singleton, \(A_p\) is linear.  Define its
positive and negative coefficient masses by
\begin{equation}
  C_p^+:=\sum_{A\in\mathcal D_p,\,c_A>0}c_A,
  \qquad
  C_p^-:=\sum_{A\in\mathcal D_p,\,c_A<0}|c_A|,
  \qquad
  T_p:=\max\{C_p^+,C_p^-\}.
...
\end{equation}
Then
\(\max_{x\in\mathbb B^n}A_p(x)=C_p^+\),
\(\min_{x\in\mathbb B^n}A_p(x)=-C_p^-\), and hence
\(\|A_p\|_\infty=T_p\).  The extrema are attained by activating precisely
the coordinates with positive or negative coefficients, respectively.

\begin{proposition}[Exact cubic mixed-sign reduction]
\label{prop:cubic-mixed-sign}
The representation in Eq.~\eqref{eq:cubic-mixed-objective} satisfies
\begin{equation}
  f(x)=\min_{y\in\mathbb B^{|P_\rho|}}F_{\rho,M}(x,y)
  \quad\text{for all }x\in\mathbb B^n
  \label{eq:cubic-pointwise-exactness}
\end{equation}
if and only if
\[
  M_p\geq T_p
  \qquad\text{for every }p\in P_\rho.
\]
If every inequality is strict, its unique auxiliary minimiser is
\[
  \phi_\rho(x):=(x_ix_j)_{p=\{i,j\}\in P_\rho}.
\]
\end{proposition}

For fixed \(x\), minimisation over the variables \(y_p\) separates.  The
proposition therefore follows from
Proposition~\ref{prop:sharp-aggregate-residual} and
\(\|A_p\|_\infty=T_p\).  Hence a design \(\sigma=(\mathcal D,\rho)\), together
with canonical penalties
\begin{equation}
  M_p=T_p+\varepsilon_p,
  \qquad \varepsilon_p>0\ \text{fixed},
  \label{eq:canonical-cubic-penalty}
\end{equation}
defines a pointwise-exact quadratic--cubic representation with a unique lift.
It uses \(|P_\rho|\) auxiliaries and is quadratic exactly when
\(\mathcal D=\mathcal C_3(f)\).  Let
\(\Sigma_{\mathrm{ex}}^\varepsilon\) denote the finite family of all such
representations.  Appendix~\ref{app:cubic-structure} records the auxiliary-reuse
structure and its pair-cover characterisation.


\section{Resource-Aware Representation Selection}
\label{sec:selection}

Section~\ref{sec:cubic-quadratization} establishes pointwise exactness at
arbitrary degree. To make representation selection finite and explicit, we
now work with the cubic family \(\Sigma_{\mathrm{ex}}^\varepsilon\) of
Section~\ref{sec:cubic-specialisation}. Thus exactness is fixed a priori;
only the exact representation is optimised.

Polynomial reduction can change compiled QAOA resources
~\cite{Schmidbauer2024}. Existing compiler-aware selection mainly concerns
fully quadratized formulations~\cite{Herrman2021,Schmidbauer2026,Rovara2026};
here each cubic term has a native or reducing action, so the family also
contains exact mixed-locality representations.

\subsection{Representation-selection problem}
\label{sec:selection-problem}

A design \(\sigma=(\mathcal D,\rho)\in\Sigma_{\mathrm{ex}}^\varepsilon\)
uses the canonical penalties in Eq.~\eqref{eq:canonical-cubic-penalty}.
Its pointwise exactness and unique consistent lift follow from
Proposition~\ref{prop:cubic-mixed-sign}.  Fix a deterministic protocol
\(\Pi=(\mathcal A,\mathcal C,\mathcal S,\operatorname{Agg})\), where
\(\mathcal S\) is the optional compiler-seed bundle and
\(\operatorname{Agg}\) its declared aggregation rule.  We abbreviate the
compiled metrics of Section~\ref{sec:resource-relaxation-data} as
\(G_{2q,\sigma},D_{2q,\sigma},S_\sigma\), with
\(a_\sigma=|P_\rho|\) and \(M_{\max,\sigma}=0\) for the native design.

For scales \(s:=(s_a,s_G,s_D,s_M)\in\mathbb R_{>0}^4\) and weights
\(w:=(w_a,w_G,w_D,w_M)\in\mathbb R_{\geq0}^4\) satisfying
\(\mathbf 1^\top w=1\), define
\begin{equation}
J_{\mathrm{res}}(\sigma)
=
...
The quantities \(G_{2q,\sigma}\) and \(D_{2q,\sigma}\) are measured after
synthesis and routing, because compiled cost-layer complexity also depends on
gadget ordering and connectivity
~\cite{Verchere2023,Weidenfeller2022,Pelofske2024,Rovara2026}.

Let \((\mu,q)\) be fixed relaxation data shared by all designs, and introduce
the diagnostic product law
\(
...
\), with all coordinates independent.  Define
\begin{equation}
...
\end{equation}
where \(s_{\mathrm{fib}}>0\) is independent of \(\sigma\).  Non-negativity
holds because \(F_\sigma(x,y)\geq f(x)\) pointwise.  Appendix~\ref{app:fibre-risk}
gives the closed form.

This product law is a diagnostic constructed from the chosen marginals;
it does not assert that the relaxation itself supplies an independent joint distribution.
...
\begin{array}{c}
N_\sigma\le Q_{\max},\quad
G_{2q,\sigma}\le B_G,\quad
D_{2q,\sigma}\le B_D,\\
M_{\max,\sigma}\le B_M,\quad
\widehat E_{\mathrm{fib}}(\sigma)\le\tau_{\mathrm{fib}}
\end{array}
\right\}.
\label{eq:selection-feasible-set}
\end{equation}
Any further declared constraint is incorporated by intersecting
\(\mathcal F_{\mathrm{sel}}\) with its feasible set.  The selection problem is
\begin{equation}
J_{\mathrm{res}}^\star
:=
...
\end{equation}
When \(\mathcal F_{\mathrm{sel}}\neq\varnothing\), finiteness of
\(\Sigma_{\mathrm{ex}}^\varepsilon\) guarantees that the minimum is attained.
Independently of \(w\), the algorithm retains non-dominated evaluated vectors
\(r_{\mathrm{sel}}(\sigma)\).  Appendix~\ref{app:selector-details} defines
their dominance relation and NSGA-II crowding tie-break
~\cite{DebEtAl2002}.
...
The selection problem has two tasks: finding a feasible representation with a
small resource score, and determining how far that score can lie above the
minimum.  Algorithm~\ref{alg:selector} uses a Pareto-beam search for the first
task and an optional branch-and-bound search for the second.  Both search the
same finite family of pointwise-exact designs; feasibility additionally
requires the budgets in Eq.~\eqref{eq:selection-feasible-set}.
...
\qquad
0\leq U_J-J_{\mathrm{res}}^\star\leq U_J-L_J^{\mathrm{glob}}.
...
\end{proposition}
The upper bound is supplied by the incumbent; the lower bound follows because
an unevaluated optimal design remains below an open state with an admissible
bound.  Appendix~\ref{app:anytime-certificate-proof} gives the proof.
For example, \(U_J=10\) and \(L_J^{\mathrm{glob}}=8\) place the optimum in
\([8,10]\), so the incumbent is at most two above it; they do not establish
that a score of eight is attainable.  The certificate remains valid if the
search stops early, provided frontier coverage is preserved.  Cache and
stopping rules are given in Appendix~\ref{app:selection-implementation}.
...
Fix a representation \(\sigma=(\mathcal D,\rho)\in\Sigma_{\mathrm{ex}}^\varepsilon\) with canonical penalties. We place the original qubits first, followed by the auxiliary qubits in a fixed ordering of \(P_\rho\).

To construct relaxation-informed warm starts~\cite{Egger2021,TateSDP2023},
we use the same relaxation pseudo-moments $(\bar\mu,\bar q)$ across all
representations. Here $\bar\mu_i$ and $\bar q_{ij}$ represent relaxed
first and pair moments, respectively, and need not arise from a common
joint distribution. For a fixed $\delta\in(0,1/2)$, define
\[
  \operatorname{clip}_\delta(t)
  :=\min\{1-\delta,\max\{\delta,t\}\}.
\]
Clipping keeps probabilities away from zero and one, preventing
the bit-flip component of the matched mixer from vanishing.
The initial probabilities for the original qubits are then
$\mu_i:=\operatorname{clip}_\delta(\bar\mu_i)$.


For each active pair \(p=\{i,j\}\), set
\[
q_p:=\begin{cases}
\operatorname{clip}_\delta(\bar q_{ij}),&\text{if the relaxation supplies }\bar q_{ij},\\
\mu_i\mu_j,&\text{otherwise}.
\end{cases}
\]
The independence fallback is not clipped again: both choices lie in
\((0,1)\).  When comparing policies, write
\(q_p^{\mathrm{pair}}=\operatorname{clip}_\delta(\bar q_{ij})\) (when available)
and \(q_p^{\mathrm{ind}}=\mu_i\mu_j\).  For \(t\in[0,1]\), define
\[
  |\phi(t)\rangle:=\sqrt{1-t}\,|0\rangle+\sqrt t\,|1\rangle
  =R_y(2\arcsin\sqrt t)\,|0\rangle .
\]
...
\begin{equation}
  ...
  \label{eq:ws-state}
\end{equation}
Its computational-basis distribution is the independent product law of
Section~\ref{sec:selection-problem}; its expected encoded-energy excess is
\(E_{\mathrm{fib}}(\sigma;\mu,q)\).
...
For \(\omega\in(0,1)\), let \(\theta_\omega:=2\arcsin\sqrt\omega\) and
define the matched single-qubit mixer~\cite{Egger2021,TateCustom2023}
\[
  h_M(\omega):=
  -2\sqrt{\omega(1-\omega)}\,X-(1-2\omega)Z
  =-R_y(\theta_\omega)ZR_y(-\theta_\omega).
\]
Thus \(h_M(\omega)\) has eigenvalues \(\{-1,1\}\), with unique ground
state \(|\phi(\omega)\rangle\). On the full register, set
\begin{equation}
  H_{M,\sigma}^{\mathrm{ws}}
  :=
  \sum_{i=1}^n h_M^{(i)}(\mu_i)
  +\sum_{p\in P_\rho}h_M^{(y_p)}(q_p),
  \label{eq:ws-mixer}
\end{equation}
where superscripts specify the qubit acted on, with identity elsewhere.
The summands commute and act on distinct factors, so
\(|\Phi_\sigma^{\mathrm{ws}}\rangle\) is the unique ground state, with
energy \(-N_\sigma\), where \(N_\sigma=n+a_\sigma\).
The spectral gap is \(2\).
At uniform marginals, \(|\Phi_\sigma^{\mathrm{ws}}\rangle
=|+\rangle^{\otimes N_\sigma}\) and
\(H_{M,\sigma}^{\mathrm{ws}}=-\sum_{j=1}^{N_\sigma}X_j\).
We use a common optimiser coordinate $\beta$ for cold and warm starts.
The cold mixer is $\exp(-i\beta\sum_jX_j)$, and the matched warm unitary
below is $\exp(+i\beta H_{M,\sigma}^{\mathrm{ws}})$. Thus uniform
probabilities on every qubit recover the cold unitary at the same parameters.
Equivalently, if the physical convention is
$\exp(-i\beta_{\mathrm{phys}}H_{M,\sigma}^{\mathrm{ws}})$, every objective
call uses $\beta_{\mathrm{phys}}=-\beta$; the initialisation and optimiser
coordinates retain the cold convention.
Define
\[
  H_{F_\sigma}:=F_\sigma(N_1,\ldots,N_{N_\sigma}),
  \qquad \text{with }N_j\text{ as in Section~\ref{sec:pb-diagonal}},
\]
\[
  U_{F_\sigma}(\gamma):=e^{-i\gamma H_{F_\sigma}},
  \qquad
  U_{M,\sigma}^{\mathrm{ws}}(\beta):=e^{+i\beta H_{M,\sigma}^{\mathrm{ws}}}.
\]
For \(r\in\mathbb Z_{\geq1}\) and
\((\boldsymbol\gamma,\boldsymbol\beta)\in\mathbb R^{2r}\),
the ansatz~\cite{Farhi2014,Egger2021,TateCustom2023} is
\begin{equation}
  |\psi_{r,\sigma}(\boldsymbol\gamma,\boldsymbol\beta)\rangle
  =
  U_{M,\sigma}^{\mathrm{ws}}(\beta_r)U_{F_\sigma}(\gamma_r)
  \cdots
  U_{M,\sigma}^{\mathrm{ws}}(\beta_1)U_{F_\sigma}(\gamma_1)
  |\Phi_\sigma^{\mathrm{ws}}(\mu,q)\rangle .
\end{equation}
$r=0$ denotes the initial state.
The local mixer can move amplitude from the consistent subspace
to inconsistent assignments. The chosen penalties make these
assignments higher in energy than their consistent counterparts,
but do not prevent the dynamics from reaching them or guarantee
consistent measurement outcomes.
Appendix~\ref{app:warm-start-circuits} gives the single-qubit realisations.


\subsection{Cost layer and consistency metric}
\label{sec:cost-consistency}

Write \(H_{F_\sigma}=\sum_{T\subseteq[N_\sigma]}h_T^{(\sigma)}Z_T\),
with \(Z_T:=\prod_{j\in T}Z_j\).
Since \([Z_T,Z_{T'}]=0\), the cost layer factorises exactly:
\[
  U_{F_\sigma}(\gamma)
  =\prod_{T\subseteq[N_\sigma]}
    e^{-i\gamma h_T^{(\sigma)}Z_T}.
\]
Thus no Trotter approximation is required
~\cite{Hadfield2021,CampbellDahl2022,Verchere2023}.
Native interactions and penalty terms use the same declared compiler
protocol; Appendix~\ref{app:reference-compiler} specifies the parity gadgets,
and Appendix~\ref{app:warm-start-circuits} records the mixer and penalty phases.
...
\subsubsection{A small worked example}
...
\subsection{Structural coordinates and benchmark scope}
...
\subsection{Benchmark scope and comparison rules}
...
\subsection{Exactness and penalty checks}
...
\subsection{Logical and compiled resources}
...
The selector uses the settings in \cref{tab:selector-settings}. Complete
...
\subsection{QAOA budgets and optimisation objective}
...
\subsection{Aggregation, uncertainty, and supplementary analyses}
...
We first check exactness and selection on the oracle tier, then ask whether
resource savings translate into better QAOA outcomes. Warm-start results are
analysed separately because their interpretation depends on the available
relaxation signal. Full counts and supplementary studies are collected in
...
Exhaustive checks on 60 oracle instances found no pointwise mismatch above
the analytical penalty threshold, and every minimising auxiliary assignment
was unique and consistent. At the threshold, the correct minimum value was
preserved but inconsistent auxiliary assignments could tie. Below it, explicit
failures occurred. These observations agree with the distinction between
pointwise exactness and strict auxiliary consistency in
\cref{prop:cubic-mixed-sign}; complete counts are in
Appendix~\ref{app:results-exactness}.

The Pareto beam satisfied the fibre-risk guardrail on all 60 instances and
matched the complete-space optimum in 58 cases: all 30 spin-glass instances
and 28 of 30 Max-3SAT instances. Its maximum score regret was $0.010156$
(\cref{tab:selector-oracle-validation,fig:selector-oracle-summary}). The two misses were training-labelled;
all 12 test-labelled oracle cases were matched. This is an exploratory
finite-space check under the protocol of Section~\ref{sec:benchmark-tiers}.
The greedy baseline matched all spin-glass optima, which were native in this
sample, but none of the Max-3SAT optima.
...
work and timing details are in Appendix~\ref{app:results-selector}.}
...
The separate branch-and-bound run certified all 60 instances, reaching the
enumerated optimum with final gap zero. Stored lower bounds were
non-decreasing and upper bounds non-increasing. No run was pruned by the
conservative resource bound, although 26 used certified capacity pruning.
Thus the runs validate the terminal certificate on small search spaces;
they do not demonstrate an efficiency advantage over enumeration or scaling
to larger instances. Timings and gap traces are in
\cref{tab:selector-bnb-certification,fig:selector-bnb-gap}.

The sensitivity study changed relatively few choices when beam width or
penalty margin was varied moderately. Resource weights and the fibre
threshold had larger effects, indicating that the declared resource trade-off
and acceptable excess matter more than these modest search-width changes.
The guardrail removed all 12 recorded violations in the 48-case calibration
ablation (\cref{tab:selector-guardrail-ablation}).
\Cref{fig:selector-sensitivity-addendum} summarises choice stability;
Appendix~\ref{app:results-selector} gives the full sweep. These checks did not rerun QAOA.

\subsection{Resource savings and structural dependence}
\label{sec:results-resources}
\label{sec:results-regimes}

The resource comparison uses all 180 compilation-tier instances, with 90 per
family, and pools split labels as specified in Section~\ref{sec:benchmark-tiers}.
All scheduled all-to-all compilations succeeded. The paired differences in
\cref{tab:e2-all-to-all-paired} show that selective reduction lowers the
Max-3SAT median gate count by 24 and depth by 61 relative to native. For spin
glass it lowers depth by 18 but adds seven gates. Savings therefore depend
on both the instance family and the resource being measured. Marginal levels
and selector diagnostics are given in Appendix~\ref{app:results-resources}.
...
Width is a separate cost. On the 12-qubit grid, 108 native designs fitted,
compared with 56 selective and 21 fully reduced designs
(\cref{tab:e2-sparse-capacity}). Only 21 instances fitted in all four
representations: 12 Max-3SAT and nine spin-glass cases. Comparing each
representation's success-only median would mix different subsets, so the
direct grid comparison uses this common set.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}lrr@{}}
        ...
        Selective       & 56/180  & 31.1\%\\
        Matched random  & 56/180  & 31.1\%\\
        \bottomrule
    \end{tabular}
\end{table}
...
On common-fit Max-3SAT instances, selective reduction saves a paired median
of 10 gates and 21 depth units relative to native, whereas the matched-random
control increases gate count. The choice of substitutions therefore matters
beyond the auxiliary count alone. In the spin-glass subset, eight of nine
selective designs are native, explaining the zero selective and random
medians; full reduction increases every listed resource. The common subset
contains 17 training, one validation, and three test-labelled instances, so
these are descriptive device-feasible results.

The added line, ring, and heavy-hex compilations reproduce the Max-3SAT
selective-versus-random pattern: selective depth is lower on every common-fit
instance, and gate count is lower on all line and ring cases and 54 of 55
heavy-hex cases. Spin-glass medians are zero, although some instances improve.
\Cref{fig:e2-additional-topologies} summarises the paired mean changes;
Appendix~\ref{app:results-connectivity} gives the full topology results.
Because the 19-qubit heavy-hex graph admits more instances than the 12-qubit
maps, these differences do not isolate connectivity from capacity.

The sparsity--pair-reuse map places these resource changes against the
coordinates of Section~\ref{sec:benchmark-generation}:
Figure~\ref{fig:e5-regime-map-2d}(a)--(c) shows all-to-all results and
(d)--(f) the grid results.
The separate regime screen favoured selective reduction over matched random
in all 20 estimable Max-3SAT depth endpoints and 19 of 20 gate endpoints;
spin-glass resource endpoints were mostly classified as having little effect.
These are endpoint counts under a within-endpoint stability rule, not
independent observations or population-level confidence statements.
Some structural bins contain only one or two observations, and grid capacity
excludes many cases. The evidence supports a structure-dependent trade-off,
not a universal rule that greater pair reuse guarantees better performance.
Complete regime counts are in Appendix~\ref{app:results-regimes}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{output/pdf/figure_e5_regime_map_2d.pdf}
    \caption{Two-dimensional representation-regime map from the existing
...
\subsection{QAOA performance under matched budgets}
\label{sec:results-qaoa}

The noiseless comparison uses 14 test-labelled instances, seven per family,
with equal depths $p=1,2$ and gate budgets $B=128,256$. Every result is scored
on the original objective. Across the scheduled data, native has the lowest
mean objective at all four budgets in both families
(\cref{tab:e3-qaoa-summary}); circuit savings do not produce a general
finite-depth QAOA advantage.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        & Selective      & 0.763 & 0.654 & 0.798 & 1.169\\
        & Matched random & 0.605 & 0.581 & 0.718 & 0.815\\
        \addlinespace
        Max-3SAT
...
Uncertainty remains substantial. Five of the eight selective-minus-native
95\% intervals include zero: all four spin-glass settings and Max-3SAT at
$B=128$. For Max-3SAT at $p=2$, the paired difference is
$1.935\;[0.597,3.273]$, favouring native, whereas at $B=128$ it is
$0.040\;[-1.013,1.093]$. The two families are analysed separately because
their objective scales differ.

The 128-gate comparison also contains initial-state baselines: 41 of 112
design/draw groups, or 123 restart rows, have $p=0$ because no complete cost
layer fits. These are gate-budget failures, not width failures or ordinary
QAOA optimisation runs. Requiring all representations and all five random
draws to fit at least one layer leaves five spin-glass and three Max-3SAT
instances. On this strict common subset, selective-minus-native differences
are $-0.072\;[-0.544,0.400]$ and $0.982\;[-0.890,2.854]$, respectively;
both intervals include zero. \Cref{tab:e3-b128-fit,fig:e3-secondary-metrics}
give the zero-layer breakdown and secondary diagnostics. Encoded energy and the
auxiliary-inconsistency rate $R_{\mathrm{inc}}$ are treated as diagnostics
rather than primary performance measures.

\subsection{Warm-start benefits and their limits}
\label{sec:results-warmstart}

The coordinate-corrected comparison uses the frozen instances, archived LP
probabilities, selected designs, and budgets. All 480 targeted tasks were
reoptimised. The original E4 schedule contains 4,320 tasks: 2,631
positive-depth warm and 1,221 positive-depth cold tasks were reoptimised,
and 468 zero-layer initial states were re-evaluated and validated for reuse.
All 4,800 task identities are accounted for, with no unresolved failures
or missing results. The environment and reuse decision are documented in
Appendix~\ref{app:warmstart-correction-provenance}.

In the original E4 sample, 11 of 14 instances satisfy the weak-signal rule
$\operatorname{median}_i|\widehat\mu_i-1/2|\leq0.05$. This diagnoses weak
original-variable bias; it does not by itself imply an unbiased auxiliary
register. The common selective active-auxiliary comparison contains only
two spin-glass and seven Max-3SAT instances. All eight primary pair-minus-cold
intervals include zero. At $B=128$, both selected spin-glass cases and two
Max-3SAT cases have zero layers and remain labelled in the scheduled comparison.
\Cref{tab:e4-warmstart-common,fig:e4-signal-diagnostics} gives the original-sample
results and unchanged signal diagnostics.

\label{sec:results-strong-bias-warmstart}
The targeted study uses the original 20 strongest-screened instances, ten
per family, selected from relaxation information alone. Its 480 noiseless
runs use $p=1,2$ and contain no zero-layer rows. The original E4 and targeted
probabilities retain their distinct LP optimum-selection provenance, and
the studies are analysed separately. The unchanged
\cref{fig:e4-strong-bias-marginals} shows the targeted single-variable bias
and pair dependence.
...
\midrule
Spin glass & $p=1$ & $0.003\;[-0.273,0.279]$ & $-0.109\;[-0.329,0.111]$ & $0.020\;[-0.239,0.279]$\\
Spin glass & $p=2$ & $0.115\;[-0.047,0.277]$ & $-0.020\;[-0.237,0.197]$ & $-0.001\;[-0.310,0.308]$\\
Max-3SAT & $p=1$ & $-6.720\;[-9.271,-4.168]$ & $-0.061\;[-0.277,0.155]$ & $0.082\;[-0.033,0.196]$\\
Max-3SAT & $p=2$ & $-6.797\;[-9.140,-4.455]$ & $-0.476\;[-0.845,-0.106]$ & $-0.165\;[-0.673,0.343]$\\
\bottomrule
\end{tabular}}
\end{table}

For targeted Max-3SAT, all three warm policies improve on cold start
(\cref{fig:e4-strong-bias-warmstart}). The corrected pair-minus-cold mean
differences are $-6.720$ at $p=1$ and $-6.797$ at $p=2$, with intervals
below zero. Mean optimal-solution probability increases from $0.0370$
under cold start to $0.6001$ under pair warming at $p=1$, and from
$0.0413$ to $0.6262$ at $p=2$. These probabilities are secondary diagnostics;
the optimised loss remains the original-objective expectation.
Spin-glass objective differences remain uncertain.

At $p=2$, targeted Max-3SAT has a pair-minus-original-only mean difference
of $-0.476$, with interval $[-0.845,-0.106]$. Its $p=1$ interval includes zero.
Both pair-minus-independence intervals include zero. Thus the results retain
a limited exploratory benefit over original-only warming at one depth,
but do not establish a stable additional benefit of pair moments over the
independence closure. The screened sample does not estimate performance on
ordinary random instances; three selected spin-glass cases remain below the
single-variable signal threshold. The limited noise study is reported
separately in Appendix~\ref{sec:results-noise}.
...
The experiments answer three related but different questions.  First, the
reduction is algebraically exact when the stated penalty rule is used.  Second,
the representation can change register width, two-qubit gate count, depth,
and routing cost.  Third, a representation with a smaller circuit does not
necessarily give a better QAOA objective.

The exactness result is the clearest.  No pointwise error was found above the
penalty threshold, including for mixed selective representations, while
lowering the penalty produced explicit mismatches.  The resource results were
more dependent on the problem family.  Selective reduction gave negative paired
median changes in gate count and depth for Max-3SAT on the all-to-all topology.  For the
spin-glass instances, it reduced depth but increased gate count.  This is why
we report width, gate count, depth, and routing separately.

The noiseless QAOA experiment adds an important negative result.  The native
representation had the best mean original-objective value for both families
over the full scheduled data.  At the 128-gate budget, however, that comparison
includes zero-layer baselines.  Restricting to cases where every representation
fits at least one layer leaves only five spin-glass and three Max-3SAT
instances; the selective spin-glass mean is then slightly below native but its
paired interval includes zero.  Selective reduction can therefore be useful as
a controlled circuit representation, but the present data do not support a
general claim that it improves QAOA solution quality.

\subsection{Dependence on structure and connectivity}

The two benchmark families differ in how their cubic terms share variable
pairs.  In Max-3SAT, one substituted pair can sometimes be reused by several
terms.  This gives an auxiliary variable more chance to repay its cost.  The
E5 counts are consistent with this explanation: the selective Max-3SAT choice
usually improved gate count and depth relative to a random choice with the
same auxiliary count.  The spin-glass resource effects were smaller.

Connectivity changes the comparison again. On the original 12-qubit grid,
only 21 instances fitted in all four representations. Capacity failures are
therefore kept separate from gate, depth, and routing comparisons, which use
a common instance subset.

The two-dimensional map in \cref{fig:e5-regime-map-2d} is descriptive rather
than a general predictive rule.  It places the observed resource effects on
the pair-reuse--sparsity plane and separates the two tested connectivity
settings, but the sparse row contains only nine common-feasible instances.
Some regions contain only one or two observations.  More instances and more
device graphs would be needed to turn this pattern into a reliable selection
rule.
The additional compilation study reduces the dependence of the resource
conclusion on that one sparse graph.  The Max-3SAT saving relative to the
matched-random control appears on the line, ring, and heavy-hex maps as well as
on the original grid.  The spin-glass comparison remains dominated by ties.
However, the common-fit sample changes with the topology, and the heavy-hex
graph also has a larger capacity than the 12-qubit maps.  The study therefore
shows that the qualitative Max-3SAT pattern is repeated across several
compiler targets, but it does not separate every connectivity effect from
capacity and embedding effects.

\subsection{Warm-start and noise interpretation}

The archived relaxation supplied bounded first and pair pseudo-moments,
but 11 of the 14 original E4 instances had weak original-variable bias.
After coordinate correction, every primary pair-minus-cold interval on the
common active-auxiliary subset still contains zero. The small original sample
therefore leaves the warm-start effect uncertain.

On the strongest-screened Max-3SAT subset, all three warm policies improve
on cold start. At $p=2$, pair warming also improves over original-only
warming in an unadjusted exploratory contrast, while the $p=1$ contrast
and both pair-versus-independence contrasts remain uncertain. These results
are consistent with useful relaxation information in this subset, but do not
isolate a general advantage attributable to direct auxiliary pair moments.
The spin-glass addendum also leaves the effect uncertain.

The correction uses a different numerical environment from the archive,
which required uniformly rerunning the positive-depth cold controls.
Consequently, changes between historical and corrected optimisation outcomes
cannot be attributed solely to the mixer. Recomputing the historical E4
rows with the same instance-level Student-$t$ method also gives eight primary
pair-minus-cold intervals containing zero. The correction supports the
limited conclusions above rather than an environment-independent causal
estimate of the sign-convention change.

The supplementary noise results in Appendix~\ref{sec:results-noise}
also require a careful baseline.  Full reduction changed
less from its own noiseless value, but it started from a worse objective and
had high auxiliary inconsistency.  It would therefore be misleading to call
it the best or most robust representation from degradation alone.  The small
noise study shows that representation affects both starting quality and noise
response, but it does not provide a universal ranking.

\subsection{Limitations of the numerical study}

The general noiseless QAOA comparison still contains only 14 instances, and
the noise study contains 18.  The 20-instance addendum is a selective-only,
signal-enriched warm-start experiment and cannot be pooled with the original
four-representation E3 comparison.  The $B_{\mathrm{2q}}=128$ E3 setting also
retains 123 zero-layer bookkeeping rows; these are reported separately as
budget-capacity failures rather than ordinary optimisations.

The certification and selector-ablation studies cover only the 60 small
oracle instances.  They verify the implementation and show sensitivity under
the tested settings, but they do not establish branch-and-bound or beam-search
scaling on large instances.  The additional coupling maps extend the
compilation study only; QAOA and noise simulations remain under the original
topology protocol.  The common-fit sample also changes with device capacity,
especially between the 12-qubit maps and the 19-qubit heavy-hex graph.

The common-active warm-start comparison contains only two spin-glass instances
and seven Max-3SAT instances.  Most relaxation pseudo-moments are also close to
one half, so this benchmark is not a strong test of a highly biased warm start.
The targeted warm-start addendum contains only ten instances per family and
is deliberately enriched for relaxation signal.  Seventeen of the 20 selected
instances exceed the frozen single-variable signal threshold. The
pair-versus-original-only improvement at $p=2$ is exploratory and does not
establish a general auxiliary advantage; pair-versus-independence evidence
remains uncertain. The corrected original E4 study restores the first
primary optimum returned by HiGHS, whereas the corrected addendum restores
the deterministic optimum-face representative.  Finally,
$R_{\mathrm{inc}}$ is an auxiliary diagnostic rather than the optimisation
objective, and the noise experiment uses representative simulator parameters
rather than a calibrated real device.  These limitations
do not affect the pointwise exactness result, but they keep the QAOA,
warm-start, regime, and noise conclusions modest.

\subsection{Scope of the design search}
\label{sec:discussion-search-scope}

The search is intended to find useful exact representations under a fixed
budget, rather than to solve a new large classical optimisation problem.  Its
output depends on the declared resource weights, compiler settings, and
feasibility limits.  A returned selective design should therefore be
understood as the best design found under that frozen protocol.  When a
certified lower bound is available we report the remaining gap; otherwise we
do not describe the incumbent as globally optimal.


\subsection{Extensions beyond cubic objectives}

The one-step exactness result applies beyond cubic functions, but higher-degree
objectives require a sequence of substitutions and introduce more design
choices.  Substitution order can then change auxiliary reuse, penalty size,
and compiled resources.  Extending the numerical search to these cases is a
natural next step, but it is outside the present experimental scope, which is
kept to cubic Max-3SAT and spin-glass benchmarks.
...
We introduced a selective way to reduce cubic pseudo-Boolean objectives.  Some
cubic terms remain native, while others are replaced using shared auxiliary
variables, with each penalty accounting for all terms that use the same
auxiliary.  On the 60 oracle-tier instances, the reduced objectives were
pointwise exact above the analytical threshold, including for mixed selective
designs.  At the threshold the value remained exact but auxiliary ties
appeared, while below the threshold explicit failures were found.  On the same
oracle tier, branch-and-bound certified zero remaining gap in all 60 cases and
reproduced the complete-enumeration optimum.

The preferred representation depends on both the problem and the resource
being measured.  For Max-3SAT on the all-to-all topology, selective reduction
lowered the paired median two-qubit gate count and depth relative to the native
representation, while using fewer auxiliaries than full reduction.  For the
spin-glass family, it lowered depth but increased gate count.  The line, ring,
and heavy-hex results repeat the Max-3SAT saving relative to matched-random
reduction, but also show that hardware capacity changes the set of instances
that can be compared.  The selector was fairly stable under moderate changes
in beam width, while the fibre threshold and resource definition had a larger
effect.

Lower circuit cost did not give a general improvement in noiseless QAOA.  The
native representation had the best mean original-objective value in both
families over the full scheduled data, and the smaller common-fit comparison at
128 gates was inconclusive.  On the deliberately signal-enriched Max-3SAT set,
the coordinate-corrected relaxation-informed warm starts improved on cold
start. Pair warming improved over original-only warming at $p=2$ in an
exploratory comparison, but not clearly over the independence closure.
The original E4 pair-versus-cold comparisons, spin-glass warm-start results,
and limited noise results remain uncertain.  We
therefore view selective reduction as a controlled representation choice, not
as a universally better encoding.  Its main value is that it allows exactness,
auxiliary cost, and compiled resources to be balanced for a stated problem and
hardware setting.
...
The executable scripts, source manifests, environment information, and
instructions used to reproduce the reported summaries are available in the
project GitHub repository at
\url{https://github.com/rrocy6/urss-selective-locality-reduction}.  The numerical outputs are kept
in the original E1--E6 archive, the four-part addendum, and a separate
warm-start coordinate-correction archive. The original archives remain
preserved; warm-start performance tables and figures in this manuscript use
the corrected outcomes.  Appendix~\ref{app:reproducibility}, especially
\cref{tab:result-provenance}, states which results come from each group.
...
The repository contains the correction audit and summary records; publication
of the complete raw archive as a release attachment is pending.

The certification, connectivity, selector-sensitivity, E3-diagnostic, and
regime-map figures retain their archived inputs. Warm-start performance
figures use the corrected rows, while the unchanged LP signal figures retain
the original diagnostics. No experiment was rerun solely to produce a figure.  An archival DOI and
...
This appendix specifies the quantities and routines used in Section~\ref{sec:selection}.
...
Use the independent product law of Section~\ref{sec:selection-problem}.
For $p=\{i,j\}$, write $t_p=\mu_i\mu_j$.  The mismatch probability and
expected penalty are
\begin{equation}
\nu_p=q_p(1-t_p)+(1-q_p)t_p,
\qquad
\ell_p=t_p+q_p(3-2\mu_i-2\mu_j).
...
\begin{equation}
E_{\mathrm{fib}}(\sigma;\mu,q)
=\sum_{p\in P_\rho}\bigl[(q_p-t_p)A_p(\mu)+M_p\ell_p\bigr],
\qquad
A_p(\mu)=\sum_{A\in\mathcal D_p}c_A\prod_{k\in A\setminus p}\mu_k.
\label{eq:fibre-excess-full}
\end{equation}
Indeed, subtract $f$ from Eq.~\eqref{eq:cubic-mixed-objective} and take
expectations term by term.  When $q_p=t_p$, the interaction correction is zero.
...
Use the action tree from Section~\ref{sec:search-certification}, with a fixed
...
Qiskit Aer~0.17.2, and SciPy~1.18.1. The warm-start coordinate correction
...
level~1, SABRE layout and routing, the translator pass, and no scheduling.
...
selection was made.  The relaxation was solved with SciPy's HiGHS interface
...
initial parameters, seeds, compiled budgets, and optimisation loss. The
132 materialised designs and all 4,800 historical saved-parameter replays
passed validation before the corrected comparisons were interpreted.
The full-register null-control tests also passed at the fixed tolerances
...
availability statement.  The tables in the main text are therefore summaries rather than replacements for
the underlying instance-level records. The E3 interval statements and E6
tables retain their archived instance-level re-aggregation. Original E4 and
targeted warm-start performance use the new coordinate-corrected task rows,
...
\centering
\small
\caption{Origin of the numerical results used in this paper.  The original
E1--E6 archive, four-part addendum, and warm-start coordinate correction
are kept separate. Warm-start paths are relative to the correction execution
directory; other paths are relative to their stated source archive.}
...
Base resource comparison & Original E2 & \texttt{e2\_compiled\_resources\_by\_seed.csv} \\
Noiseless QAOA and secondary metrics & Original E3 & \texttt{e3\_qaoa\_runs.csv} \\
Original E4 warm-start comparison & Coordinate correction & \texttt{original\_e4/corrected\_runs.csv} \\
Structural regime analysis & Original E5 & \texttt{e5\_regime\_instance\_level.csv} \\
Limited noise study & Original E6 & \texttt{e6\_noise\_runs.csv} \\
Branch-and-bound certificates and traces & Four-part addendum & \texttt{certification/} \\
Line, ring, and heavy-hex compilation & Four-part addendum & \texttt{topologies/additional\_topology\_compiled\_by\_seed.csv} \\
Selector sensitivity study & Four-part addendum & \texttt{selector\_ablation/selector\_ablation\_raw.csv} \\
Signal-enriched warm-start study & Coordinate correction & \texttt{targeted/corrected\_runs.csv} \\
\bottomrule
\end{tabular}}
\end{table}
...
Full quadratization uses a minimum-cardinality pair cover of all
...
used here.  It is a sparse RLT-style, strengthened recursive-McCormick
relaxation of the canonical cubic objective
\begin{equation}
...
is labelled as having little effect.
...
The threshold behaviour is summarised in Section~\ref{sec:results-exactness}.
\Cref{tab:e1-exactness-summary} gives the representation-level counts:
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{output/pdf/figure_oracle_selector_summary.pdf}
...
\begin{figure}[!htbp]
    ...
    {output/pdf/figure_certification_gap_time.pdf}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]
    {output/pdf/figure_selector_sensitivity_addendum.pdf}
...
are 145 and 178, while the median paired difference is $-24$, not $-33$.
On the grid, 4,875 of 7,200 scheduled rows are expected width failures, with
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        Max-3SAT & Native          & 12 & 0 & 178 & 119.5\\
                 ...
                 & Selective       & 18 & 5 & 145 & 56.5\\
                 & Matched random  & 18 & 5 & 182 & 94\\
        \addlinespace
        Cubic spin glass & Native         & 12 & 0 & 96  & 79\\
                         & Fully reduced  & 18 & 6 & 148 & 47\\
                         & Selective      & 14 & 1 & 115 & 63\\
                         & Matched random & 14 & 1 & 116 & 80.5\\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[!htbp]
    ...
    in each family.  Fibre risk is the normalised excess used by the v2
    guardrail; the threshold is 0.02.  Time is selector runtime in seconds.}
    \label{tab:e2-selector-diagnostics}
    % FOLLOW-UP: archived dynamic range includes the identity coefficient;
    % reconcile against eq:pauli-dynamic-range in a separate kappa amendment.
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{@{}lrrrrrrr@{}}
        ...
        \midrule
        Cubic spin glass & 19 & 9  & 7.25 & 0.00068 & 0.6995 & 4096 & 8.86\\
        Max-3SAT         & 6  & 12 & 346  & 0.01921 & 0.8156 & 4288 & 8.15\\
        \bottomrule
    \end{tabular}%
    }
\end{table}
All 180 compilation-tier selected designs satisfied the 0.02 guardrail; the
largest stored normalised excess was below 0.02.  The different median retained
cubic counts also show that the frozen selector did not apply the same amount
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        \midrule
        Line-12
        & Spin glass & 40 & $0.00$ (9) & $0.00$ (10) & $0.00$ (9)\\
        & Max-3SAT   & 16 & $-35.4$ (16) & $-33.3$ (16) & $-24.8$ (16)\\
        Ring-12
        & Spin glass & 40 & $0.00$ (9) & $0.00$ (10) & $0.00$ (9)\\
        & Max-3SAT   & 16 & $-38.3$ (16) & $-31.8$ (16) & $-26.7$ (16)\\
        Heavy-hex-19
        & Spin glass & 67 & $0.00$ (26) & $0.00$ (28) & $0.00$ (26)\\
        & Max-3SAT   & 55 & $-54.6$ (54) & $-45.0$ (55) & $-31.4$ (53)\\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]
    {output/pdf/figure_additional_topologies.pdf}
...
\end{figure}
Capacity remains part of the result.  The 12-qubit line and ring each had 40
common-fit spin-glass and 16 common-fit Max-3SAT instances, while the
19-qubit heavy-hex graph had 67 and 55.  The corresponding test-labelled
counts were only 7 and 2 on line and ring, and 13 and 9 on heavy-hex.  Since
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}llrrrr@{}}
        ...
        Spin glass & Native         & $0/7$   & 0  & $0/7$ & $7/7$\\
                   ...
                   & Matched random & $10/35$ & 30 & $2/7$ & $5/7$\\
        \addlinespace
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{output/pdf/figure_e3_secondary_metrics.pdf}
...
total.  There were 68 tested designs with active auxiliary variables, for which
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]{output/pdf/figure_e4_signal_diagnostics.pdf}
    \caption{Signal available to the E4 warm start.  Each point summarises one
    raw QAOA instance.  Circles identify cases where the selected
...
the nine-case common selective active-auxiliary subset.
...
separate. The complete corrected original E4 schedule retains 4,320 rows.
Positive-depth, zero-depth, and all-scheduled summaries for the full design
set are separately available in the correction archive.
...
$p=1$ & 2 & 0 & $-1.107$ & $-0.038\;[-8.640,8.564]$ & $-0.004\;[-8.172,8.164]$ & $-0.316\;[-4.953,4.321]$\\
$p=2$ & 2 & 0 & $-1.214$ & $0.328\;[-4.845,5.502]$ & $-0.092\;[-0.252,0.069]$ & $-0.052\;[-2.803,2.698]$\\
$B=128$ & 2 & 2 & $0.000$ & $-0.222\;[-3.046,2.601]$ & $-0.222\;[-3.046,2.601]$ & $-0.222\;[-3.046,2.601]$\\
$B=256$ & 2 & 0 & $-0.880$ & $0.070\;[-2.723,2.862]$ & $0.037\;[-2.342,2.417]$ & $-0.075\;[-4.928,4.778]$\\
\addlinespace
...
All eight primary pair-minus-cold intervals include zero. In the direct
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{output/pdf/figure_original_e4_warmstart_corrected.pdf}
...
Student-$t$ intervals. The $n_0$ annotations identify retained zero-layer
instances. The very small spin-glass subset yields wide intervals.}
...
for targeted Max-3SAT under independence are $0.6107$ and $0.6214$ at
$p=1,2$, compared with $0.6001$ and $0.6262$ under pair warming.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=\textwidth]
    {output/pdf/figure_strong_bias_marginals.pdf}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{output/pdf/figure_strong_bias_warmstart.pdf}
...
routing overhead, and QAOA objective.  Outcomes inside the declared tolerance
were labelled as ``little effect'' rather than being counted as a win or a
loss.

\begin{table}[!htbp]
    \centering
    \small
    \caption{Help, little-effect, and hurt counts for selective reduction
    relative to the matched-random control.  The rows count estimable endpoint
    comparisons, not independent benchmark instances.}
    \label{tab:e5-regime-summary}
    \begin{tabular}{@{}llrrrr@{}}
        \toprule
        Family & Endpoint & $N$ & Help & Little effect & Hurt\\
        \midrule
        Cubic spin glass & Two-qubit depth  & 25 & 8 & 17 & 0\\
        Cubic spin glass & Two-qubit gates  & 25 & 3 & 22 & 0\\
        Cubic spin glass & Routing overhead & 25 & 1 & 24 & 0\\
        Cubic spin glass & QAOA objective   & 28 & 0 & 25 & 3\\
        \addlinespace
        Max-3SAT & Two-qubit depth  & 20 & 20 & 0  & 0\\
        Max-3SAT & Two-qubit gates  & 20 & 19 & 1  & 0\\
        Max-3SAT & Routing overhead & 20 & 1  & 19 & 0\\
        Max-3SAT & QAOA objective   & 28 & 1  & 13 & 14\\
        \bottomrule
    \end{tabular}
\end{table}

Of 272 scheduled instance--metric endpoints, 191 were estimable: 53 helped,
121 had little effect, and 17 hurt. In the QAOA rows, $N=28$ denotes seven
instances at four budgets. Classification follows
Appendix~\ref{app:experimental-statistics}; these are endpoint counts,
not independent-instance confidence statements.

The other 81 scheduled endpoints were expected sparse-device infeasibilities.
...
\begin{tabular}{@{}llrrrrr@{}}
        ...
        & Selective       & -2.735 & -0.642 & -0.112 & 2.623 & 0.179\\
        & Matched random  & -2.653 & -0.618 & -0.093 & 2.560 & 0.178\\
        \addlinespace
        Max-3SAT
        & Native          & 7.953 & 9.305 & 9.700 & 1.748 & 0.000\\
        & Fully reduced   & 9.024 & 9.396 & 9.633 & 0.609 & 0.821\\
        & Selective       & 8.723 & 9.456 & 9.695 & 0.972 & 0.747\\
        & Matched random  & 8.364 & 9.286 & 9.645 & 1.282 & 0.742\\
        \bottomrule
    \end{tabular}}%

\end{table}

Noise worsened the mean original objective for every representation.  Full
reduction had the smallest numerical change from its own noiseless value, but
its noiseless baseline was already worse than native in both families and its
auxiliary inconsistency rate was high.  A smaller degradation therefore does
not mean that it gave the best absolute result.  For the spin-glass subset,
selective reduction kept the inconsistency rate much lower than full
reduction.  For Max-3SAT, however, the selective inconsistency rate remained
high.
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{@{}llrrrl@{}}
        ...
        \midrule
        Cubic spin glass & Noiseless & $0.317$  & $[-0.642,1.276]$ & $0.254$  & 4/0/5\\
                          & Low       & $0.056$  & $[-0.188,0.301]$ & $0.178$  & 5/0/4\\
                          & High      & $-0.025$ & $[-0.068,0.019]$ & $-0.434$ & 4/3/2\\
        \addlinespace
        Max-3SAT         & Noiseless & $0.770$  & $[0.144,1.396]$  & $0.946$  & 3/0/6\\
                          & Low       & $0.150$  & $[-0.045,0.346]$ & $0.591$  & 1/0/8\\
                          & High      & $-0.006$ & $[-0.130,0.118]$ & $-0.036$ & 5/0/4\\
        \bottomrule
    \end{tabular}
\end{table}

After within-instance aggregation, selective reduction was not clearly better
than native at either non-zero noise level.  All three spin-glass intervals
included zero.  For Max-3SAT, selective reduction was worse in the noiseless
setting, while the low- and high-noise differences were uncertain.  Its change
from noiseless to noisy operation was smaller than native for Max-3SAT, but it
also started from a worse noiseless value.  A smaller degradation should
therefore not be read as better absolute noisy performance.  With only nine
instances per family, this remains a limited exploratory robustness check.
