...
def _clean_code_commit() -> str:
    ...
    if status.strip():
        raise RuntimeError(
            "Formal E5 must run from the clean committed Step 7 implementation"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run formal Step 7 / E5 sparsity-reuse-connectivity regime map"
    )
    parser.add_argument("--config", required=True)
    parser.add_argument("--config-hash", required=True)
    parser.add_argument("--analysis-config", required=True)
    parser.add_argument("--analysis-config-hash", required=True)
    ...
    summary = run_e5_regime_pipeline(
        config_path=arguments.config,
        config_hash_path=arguments.config_hash,
        analysis_config_path=arguments.analysis_config,
        analysis_config_hash_path=arguments.analysis_config_hash,
