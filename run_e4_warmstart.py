...
def _clean_code_commit() -> str:
    ...
    if status.strip():
        raise RuntimeError(
            "Formal E4 must run from the clean committed Step 6 implementation"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run formal Step 6 / E4 warm-start and relaxation ablation"
    )
    ...
    summary = run_e4_warmstart_pipeline(
